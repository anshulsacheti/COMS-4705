...
class nmt_dynet_attention:
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, src_word2idx, src_idx2word, tgt_word2idx, tgt_idx2word, word_d, gru_d, gru_layers):
        ...
        # pulling in fasttext lookup params

        #Chinese
        chineseWords = {}
        chineseEmbeddings = []
        i=0
        with open("zh.vec") as file:
            for line in file:
                vals = line.strip().split(" ")
                chineseEmbeddings.append([float(v) for v in vals[1:]])
                chineseWords[vals[0]] = i
                i+=1

        #Map values so errors don't occur
        chineseWords["<s>"] = chineseWords["</s>"]
        chineseEmbeddings.append(chineseEmbeddings[chineseWords["<s>"]])

        # self.ref_source_embeddings = self.model.add_lookup_parameters((self.src_vocab_size, self.word_d))
        # self.ref_target_embeddings = self.model.add_lookup_parameters((self.tgt_vocab_size, self.word_d))

        self.source_embeddings = self.model.add_lookup_parameters((len(chineseEmbeddings), len(chineseEmbeddings[-1])))
        self.source_embeddings.init_from_array(np.array(chineseEmbeddings))
        self.src_word2idx = chineseWords
        self.src_idx2word = {chineseWords[k]:k for k in chineseWords.keys()}
        src_vocab_size = len(chineseWords)
        self.src_vocab_size = src_vocab_size

        #English GloVE embeddings
        englishWords = {}
        englishEmbeddings = []
        i=0
        with open("glove.6B.50d.txt") as file:
            for line in file:
                vals = line.strip().split(" ")
                englishEmbeddings.append([float(v) for v in vals[1:]])
                englishWords[vals[0]] = i
                i+=1

        #Adding <s> and </s> to vocab
        englishWords["<s>"] = englishWords["</s>"]
        englishWords["<unk>"] = englishWords["unk"]

        #Need to take embeddings for

        self.target_embeddings = self.model.add_lookup_parameters((len(englishWords), len(englishEmbeddings[-1])))
        self.target_embeddings.init_from_array(np.array(englishEmbeddings))
        self.tgt_word2idx = englishWords
        self.tgt_idx2word = {englishWords[k]:k for k in englishWords.keys()}
        tgt_vocab_size = len(englishWords)
        self.tgt_vocab_size = tgt_vocab_size
    ...
    def generate(self, src_sentence):
        ...
        while True and i<5*len(src_sentence):
            ...
            # End of sentence
            # if self.tgt_idx2word[newWordIndex]=="eos":
            #     break
