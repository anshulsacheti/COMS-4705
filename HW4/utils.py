...
def normalize(word):
    ...
    #Try to identify if string a number
    try:
        x=float(word)
        return "NUMBER"
    except ValueError:
...
class Vocabulary:
    ...
    def __init__(self, conll_path):
        ...
        self.idx2pos = list(self.pos.keys())
        self.idx2rel = list(self.rel.keys())
    ...
    def process(self, data, deterministic=False):
        ...
        #Store relevant data in list of lists
        #For each word in each sentence extract POS/Parent/DepRel
        for sent in data:
            word_tmp    = []
            pos_tmp     = []
            parent_tmp  = []
            ...
            for i in range(len(sent)):
                ...
                try:
                    tmp = self.word2idx[sent[i].form]

                    #Determine sampling
                    if deterministic:
                        word_tmp.append(tmp)
                    else:
                        prob = 0.25/(0.25+self.words[sent[i].form])
                        if np.random.rand()<=prob:
                            word_tmp.append(self.word2idx[UNKNOWN])
                        else:
                            word_tmp.append(tmp)

                #Word not in dict use UNKNOWN
                except KeyError:
                    word_tmp.append(self.word2idx[UNKNOWN])
            ...
            pos_idx.append(pos_tmp)
            parent_idx.append(parent_tmp)
            depRel_idx.append(depRel_tmp)

        #Example return set
        #test_indices, test_pos_indices, test_arcs, test_labels

        return [word_idx, pos_idx, parent_idx, depRel_idx]
